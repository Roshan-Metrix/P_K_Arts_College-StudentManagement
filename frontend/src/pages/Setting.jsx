...
const Setting = () => {
  ...
  const [dropdownPortalOpen, setDropdownPortalOpen] = useState(false);
  const [subPortalTab, setSubPortalTab] = useState("calendar");
          ...
          {/* Portal Setting */}
           {/* {user?.role === "admin" && ( */}
            <div className="w-full">
              <button
                onClick={() => {
                  if (!dropdownPortalOpen) {
                    setActiveTab("portal");
                    setDropdownPortalOpen(true);
                    setSubPortalTab("calendar");
                  } else {
                    setDropdownPortalOpen(false);
                  }
                }}
                className={`flex items-center justify-between gap-2 py-2 px-4 rounded-lg w-full text-left font-medium transition ${
                  activeTab === "portal"
              ...
              >
                <span className="flex items-center gap-2">
                  <LayoutTemplate className="w-5 h-5" /> College Portal
                </span>
                <ChevronDown
                  className={`w-4 h-4 transition-transform ${
                    dropdownPortalOpen ? "rotate-180" : ""
                  }`}
                />
              </button>
              ...
              {dropdownPortalOpen && (
                <div className="ml-6 mt-2 flex flex-col gap-1">
                  <button
                    onClick={() => setSubPortalTab("calendar")}
                    className={`text-left py-1 px-3 rounded-md text-sm ${
                  ...
                  <button
                    onClick={() => setSubPortalTab("events")}
                    className={`text-left py-1 px-3 rounded-md text-sm ${
                      subPortalTab === "events"
                        ? "bg-indigo-100 text-indigo-700 font-medium"
                        : "hover:bg-slate-100 text-slate-700"
